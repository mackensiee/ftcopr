...
from into_files import write_teams_csv, write_matches_csv
...
def main():
  ...
  for event in event_codes:
    calculate_event_opr(event, year)

def calculate_event_opr(event, year):
  ...
  calculate_oprs(teams_file_path, matches_file_path, oprs_file_path)
